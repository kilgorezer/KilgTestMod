...
static Vec2 PerspectiveCamera_GetMouseDelta(float delta) {
	...
	if ((Camera.RevGrav ? -Camera.ZoomFov : Camera.ZoomFov) < 0) {
		v.y = -v.y;
		v.x = -v.x;
	}
	return v;
...
static void OnHacksChanged(void* obj) {
	...
	if (h->ReverseGravity != Camera.RevGrav) {
		Camera.Fov = -Camera.Fov;
		Camera.ZoomFov = -Camera.ZoomFov;
	}
	Camera.RevGrav = h->ReverseGravity;
	Camera_UpdateProjection();
}

static void OnHacksStateChanged(void* obj) {}
...
void Camera_SetFov(int fov) {
	...
	Camera.Fov = Camera.RevGrav ? -fov : fov;
	Camera_UpdateProjection();
}
...
static void OnInit(void) {
	...
	Event_Register_(&UserEvents.HacksStateChanged, NULL, OnHacksStateChanged);
	...
	Camera.DefaultFov  = Options_GetInt(OPT_FIELD_OF_VIEW, -179, 179, 70);
	Camera.Fov         = Camera.RevGrav ? -Camera.DefaultFov : Camera.DefaultFov;
