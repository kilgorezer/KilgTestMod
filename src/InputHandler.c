...
const cc_uint8 Hotkeys_LWJGL[256] = {
	...
	0, 0, 0, 0, CCKEY_F13, CCKEY_F14, CCKEY_F15, CCKEY_F16, CCKEY_F17, CCKEY_F18, CCKEY_F19, CCKEY_F20, CCKEY_F21, CCKEY_F22, CCKEY_F23, CCKEY_F24,
...
cc_bool Input_HandleMouseWheel(float delta) {
	...
	if (input_fovIndex == -1.0f) input_fovIndex = Math_AbsF((float)Camera.ZoomFov);
	input_fovIndex -= delta * 5.0f;

	Math_Clamp(input_fovIndex, 1.0f, (float)Math_AbsI(Camera.DefaultFov));
	return InputHandler_SetFOV((int)(input_fovIndex * Math_Sign((float)Camera.DefaultFov)));
}
