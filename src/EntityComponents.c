...
void HacksComp_RecheckFlags(struct HacksComp* hacks) {
	...
	HacksComp_ParseFlag(hacks, "+revgrav",     "-revgrav",     &hacks->ReverseGravity);
...
void HacksComp_SetNoGravity(struct HacksComp* hacks, cc_bool revgrav) {
	if (hacks->ReverseGravity == revgrav) return;
	hacks->ReverseGravity = revgrav;
	Event_RaiseVoid(&UserEvents.HacksStateChanged);
}
...
static cc_bool Collisions_DidSlide(struct CollisionsComp* comp, struct AABB* blockBB, Vec3* size,
	...
	float yDist = (!Camera.RevGrav) ? blockBB->Max.y - entityBB->Min.y : entityBB->Max.y - blockBB->Min.y;
	struct AABB adjBB;
	...
	if (yDist > 0.0f && yDist <= comp->StepSize + 0.01f) {
		...
		if (!Camera.RevGrav) {
			...
			adjBB.Min.y = blockBB->Min.y - COLLISIONS_ADJ - size->y;
		}
...
static void Collisions_ClipYMin(struct CollisionsComp* comp, struct AABB* blockBB, struct AABB* entityBB, 
	...
	if (Camera.RevGrav) comp->Entity->OnGround = true;
	Collisions_ClipY(comp->Entity, size, entityBB, extentBB);
	comp->HitYMin = true;
}
...
void PhysicsComp_UpdateVelocityState(struct PhysicsComp* comp) {
	...
	if (hacks->Floating) {
		...
		if (!Camera.RevGrav) { dir = (hacks->FlyingUp || comp->Jumping) ? 1 : (hacks->FlyingDown ? -1 : 0); }
		else { dir = (hacks->FlyingUp || comp->Jumping) ? -1 : (hacks->FlyingDown ? 1 : 0); }
...
void PhysicsComp_DoNormalJump(struct PhysicsComp* comp) {
	...
	int dir = Camera.RevGrav ? -1 : 1;

	entity->Velocity.y = comp->JumpVel * dir;
	if (hacks->Speeding     && hacks->CanSpeed) entity->Velocity.y += comp->JumpVel * dir;
	if (hacks->HalfSpeeding && hacks->CanSpeed) entity->Velocity.y += comp->JumpVel / 2 * dir;
	comp->CanLiquidJump = false;
}
