...
LAYOUTS main_lblUpdate_N[] = { { ANCHOR_MAX,   10 }, { ANCHOR_MAX, -20 } };
...
#else
	LButton_Add(s, &s->btnOptions, 100, 35, "Options",
		SwitchToSettings, main_btnUpdates);

	LLabel_Add(s,  &s->lblUpdate,  "&eChecking..", main_lblUpdate_N);
	if (Updater_Supported) {
		LButton_Add(s, &s->btnUpdates,  0, 0, "", 
					SwitchToUpdates, main_lblUpdate_N);
	}
#endif
