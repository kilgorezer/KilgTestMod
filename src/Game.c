...
const char* const FpsLimit_Names[FPS_LIMIT_COUNT] = {
	"LimitVSync", "Limit15FPS", "Limit30FPS", "Limit60FPS", "Limit120FPS", "Limit144FPS", "LimitNone",
};
...
static const short normalDists[]  = { 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384 };
...
static void LoadOptions(void) {
	...
	Game_ViewDistance     = Options_GetInt(OPT_VIEW_DISTANCE, 8, 32767, DEFAULT_VIEWDIST);
...
void Game_SetFpsLimit(int method) {
	...
	case FPS_LIMIT_15:  minFrameTime = 1000/15.0f;  break;
	}
...
void Game_DoFrame(void) {
	if (gameRunning) {
		Game_RenderFrame();
		if (Math_Sign((float)Camera.ZoomFov) != Math_Sign((float)Camera.DefaultFov)) Camera.ZoomFov = -Camera.ZoomFov;
	} else if (tasksCount) {
		Game_Free();
		Window_Free();
	}	
